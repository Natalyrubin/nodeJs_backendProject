...
const mustLogin = (req, res, next) => {
  ...
  if (!token) return res.status(403).json({ success: false, message: 'Forbidden: you must be logged-in to view this content' })
  try {
    const payload = jwt.verify(token, JWT_SECRET)
    req.user = payload;
    return next();
  } catch (err) {
    return res.status(403).json({ success: false, message: 'Forbidden: you must be logged-in to view this content' })
  }
}
...
const allowedRoles = (allowedRoles) => {
  return (req, res, next) => {
    ...
    if (allowedRoles.includes('ownUser') && jwt.decode(req.header('x-auth-token')).id === req.user.id) hasRole = true;
    console.log(hasRole);
    console.log(req.user.id);
    console.log(jwt.decode(req.header('x-auth-token')).id);
